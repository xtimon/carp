...
// rpcPoolSize is the per-peer max pooled connections. Default raised from 8
// to 32 so a single coordinator fanning out parallel replica dispatch (RF *
// concurrent clients) doesn't immediately serialize on pool exhaustion.
// Change via SetPoolSize before starting RPC traffic.
var rpcPoolSize int64 = 32

// SetPoolSize configures the per-peer connection pool ceiling. Must be > 0.
// Pools created before the change keep their original capacity; this is
// intended to be called once at startup based on config.
func SetPoolSize(n int) {
	if n > 0 {
		atomic.StoreInt64(&rpcPoolSize, int64(n))
	}
}
...
func getPool(addr string) *rpcConnPool {
	...
	p := &rpcConnPool{
		addr:  addr,
		conns: make(chan net.Conn, atomic.LoadInt64(&rpcPoolSize)),
	}
	connPools[addr] = p
	return p
}
...
//
// Connection lifetime: handles frames in a loop so a single TCP conn can serve
// many RPCs. Closes only when ReadFrame errors (peer closed, EOF, malformed
// HMAC, deadline). This is what makes the per-peer client pool effective —
// otherwise every cached conn would be a single-use stub closed by us.
func HandleConn(conn net.Conn, h *Handler) {
	store := h.Store
	defer conn.Close()
	for {
		// Clear read deadline so an idle pooled conn can wait indefinitely
		// for the next frame. The client side carries a per-call deadline.
		...
		if err := clusterauth.WriteFrame(conn, clusterSecret, result); err != nil {
			return
		}
	}
}
